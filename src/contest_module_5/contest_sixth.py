...
def get_sum(b, lef, righ, s_tree, left, right):
    if left <= lef and right >= righ:
        return s_tree[b]
    if left >= righ or right <= lef:
        return 0
    m = (righ+lef)//2
    t_left = get_sum(2*b+1, lef, m, s_tree, left, right)
    t_right = get_sum(2*b+2, m, righ, s_tree, left, right)
    return t_left + t_right

def up(b, lef, righ, s_tree, ind_new, value):
    if righ-lef == 1:
        s_tree[b] = value
        return
    m = (righ+lef)//2
    if ind_new < m:
        up(2*b+1, lef, m, s_tree, ind_new, value)
    else:
        up(2*b+2, m, righ, s_tree, ind_new, value)
    s_tree[b] = s_tree[2*b+1] + s_tree[2*b+2]
def main():
    n = int(input())
